        ...
        match self.fn_type:
            ...
            case "bessel":
                if self.start is not None:
                    log.warning("Start parameter is ignored for bessel RBFs in PaiNN.")
                if self.trainable:
                    log.warning("Trainable parameter is ignored for bessel RBFs in PaiNN.")
                return BesselRBF(
                    n_rbf=self.n_rbf,
                    cutoff=cutoff,
                )
...
class PaiNNStudentModel(
    StudentModuleBase[dict[str, torch.Tensor], dict[str, torch.Tensor], PaiNNStudentModelConfig]
):
    @override
    @classmethod
    def hparams_cls(cls):
