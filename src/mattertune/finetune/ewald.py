"""
This module implements the Ewald summation technique for calculating long-range electrostatic interactions
We followed the Latent Ewald Summation methods proposed in (https://www.nature.com/articles/s41467-025-63852-x)
And the EwaldPotential implementation is copied from (https://github.com/BingqingCheng/cace/blob/main/cace/modules/ewald.py)
"""
...
class EwaldPluginConfig(C.Config):
    """
    Configuration for EwaldPluginModule
    """
    node_dim: int
    """ Dimension of the node features """
    
    num_layers: int = 2
    """ Number of layers in the MLP to compute latent charges """
    
    latent_dim: int = 1
    """ Dimension of the latent charges """
    
    dl: float = 2.0
    """ Grid resolution """
    
    sigma: float = 1.0
    """ Width of the Gaussian on each atom """
    
    exponent: int = 1
    """ Exponent for the potential calculation (1 for electrostatics, 6 for London dispersion) """
    
    external_field: float | None = None
    """ External field strength """
    
    external_field_direction: int = 0
    """ External field direction (0 for x, 1 for y, 2 for z) """
    
    charge_neutral_lambda: float | None = None
    """ Charge neutralization lambda parameter """
    
    remove_self_interaction: bool = True
    """ Whether to remove self-interaction term """
    
    feature_key: str = 'q'
    """ Key for the input charge features """
    
    output_key: str = 'ewald_potential'
    """ Key for the output Ewald potential """
    
    aggregation_mode: str = "sum"
    """ Aggregation mode for the output ('sum' or 'mean') """
    
    def create_model(self) -> EwaldPluginModule:
        return EwaldPluginModule(hparams=self)
    
class EwaldPluginModule(nn.Module):
    """
    This is a plugin module to add Ewald potential calculation to existing models.
    The inputs are:
    - positions: [n_node, 3]
    - cell: [batch_size, 3, 3]
    - batch: [n_node]
    - node_features: [n_node, n_feat]
    And the outputs are:
    - latent charges: [n_node, latent_dim]
    - energy correction: [batch_size]
    
    The latent charges are computed from the node features using a MLP.
    The energy correction is computed using the EwaldPotential module.
    """
    def __init__(
        self,
        hparams: EwaldPluginConfig,
    ):
        super().__init__()
        ...
        for i in range(hparams.num_layers):
            in_dim = hparams.node_dim
            out_dim = hparams.latent_dim if i == hparams.num_layers - 1 else hparams.node_dim
            self.charge_mlp.add_module(f'linear_{i}', nn.Linear(in_dim, out_dim))
            if i < hparams.num_layers - 1:
                self.charge_mlp.add_module(f'relu_{i}', nn.ReLU())
        
        self.ewald_summation = EwaldPotential(
            dl=hparams.dl,
            sigma=hparams.sigma,
            exponent=hparams.exponent,
            external_field=hparams.external_field,
            external_field_direction=hparams.external_field_direction,
            charge_neutral_lambda=hparams.charge_neutral_lambda,
            remove_self_interaction=hparams.remove_self_interaction,
            feature_key=hparams.feature_key,
            output_key=hparams.output_key,
            aggregation_mode=hparams.aggregation_mode,
        )
    ...
    def forward(
        ...
        *,
        ...
        # Compute latent charges
        latent_charges = self.charge_mlp(node_features)
        
        data = {
            'positions': positions,
            'cell': cell,
            'batch': batch,
            self.hparams.feature_key: latent_charges,
        }
        
        # Compute Ewald potential
        data = self.ewald_summation(data)
        
        energy_correction = data[self.hparams.output_key]
        
        return latent_charges, energy_correction