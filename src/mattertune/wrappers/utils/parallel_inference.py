...
class ParallizedInferenceDDP(ParallizedInferenceBase):
    """
    Torch DDP inference launcher with:
      - per-run rendezvous port isolation
      - strong process-tree cleanup (TERM -> wait -> KILL -> wait)
      - clean reinit before each retry
    """
    ...
    def __init__(
        ...
        *,
        ...
        # env
        self.cuda_env = os.environ.copy()
        self.cuda_env["CUDA_VISIBLE_DEVICES"] = ",".join(map(str, devices))
        # stability & allocator
        self.cuda_env.setdefault("NCCL_ASYNC_ERROR_HANDLING", "1")
        self.cuda_env.setdefault("NCCL_BLOCKING_WAIT", "1")
        self.cuda_env.setdefault("TORCH_NCCL_BLOCKING_WAIT", "1")
        self.cuda_env.setdefault("NCCL_DEBUG", "WARN")
        self.cuda_env.setdefault("NCCL_TIMEOUT", "120")
        self.cuda_env.setdefault("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True,max_split_size_mb:128")

        # script
        scripts_path = os.path.dirname(os.path.abspath(__file__))
        scripts_path = os.path.join(scripts_path, "multi_gpu_ddp.py")

        # base cmd (master_addr/port will be injected by _set_master_port)
        self.cmd = [
            "torchrun",
            ...
            "--lazy_init_atoms", str(lazy_init_atoms) if lazy_init_atoms is not None else "",
        ]
        if num_workers is not None:
            self.cmd += ["--num_workers", str(num_workers)]
        if using_partition:
            self.cmd.append("--using_partition")
        if inference_mode:
            self.cmd.append("--inference_mode")

        # inject rendezvous args & launch
        self._set_master_port()
    ...
    def _set_master_port(self):
        port = self._find_free_port()
        self.cuda_env["MASTER_ADDR"] = "127.0.0.1"
        self.cuda_env["MASTER_PORT"] = str(port)

        # add or update --master_addr/--master_port in self.cmd
        def _inject(flag: str, value: str):
            if flag in self.cmd:
                self.cmd[self.cmd.index(flag) + 1] = value
            else:
                # insert after "torchrun"
                self.cmd.insert(1, flag)
                self.cmd.insert(2, value)

        _inject("--master_addr", "127.0.0.1")
        _inject("--master_port", str(port))

    def _kill_tree(self, pid: int, sig):
        try:
    ...
    def _wait_tree_gone(self, pid: int, timeout: float) -> bool:
        t0 = time.time()
        while time.time() - t0 < timeout:
            try:
                p = psutil.Process(pid)
                alive = p.is_running() or any(ch.is_running() for ch in p.children(recursive=True))
                if not alive:
                    return True
            except psutil.NoSuchProcess:
                return True
    ...
    def run_inference(self, atoms_list: list[Atoms]) -> list[dict[str, torch.Tensor]]:
        # ensure live
        if not os.path.exists(self.tmp_dir) or self.proc.poll() is not None:
            self._reinit()
        ...
        try:
            ...
            while True:
                ...
                if os.path.exists(finish):
                    break
                # timeout guard (deadlock detector)
                if time.time() - t0 > 600:
                    raise Warning("ParallizedInferenceDDP takes >600s, likely deadlock.")
                time.sleep(0.02)
            ...
            # hard cleanup before surfacing error
            self.exit()
            raise e
    ...
    def exit(self):
        # try:
            ...
            # 1) ask child service to exit gracefully
            try:
                exit_signal(self.tmp_dir)
            except Exception:
                pass

            # 2) send SIGTERM to process group & whole tree
            try:
                pgid = os.getpgid(self.proc.pid)
                os.killpg(pgid, signal.SIGTERM)
            except Exception:
                pass
            self._kill_tree(self.proc.pid, signal.SIGTERM)
            ...
            if not self._wait_tree_gone(self.proc.pid, timeout=5):
                ...
                try:
                    ...
                    pass
                self._kill_tree(self.proc.pid, signal.SIGKILL)
                try:
                    self.proc.wait(timeout=3)
                except Exception:
                    pass

            # 5) final sweep: kill any process whose cmdline mentions this workspace
            for p in psutil.process_iter(["pid", "cmdline"]):
                try:
                    cmd = " ".join(p.info.get("cmdline") or [])
                    if self.tmp_dir and self.tmp_dir in cmd:
                        p.kill()
                except Exception:
                    pass                
        # except:
    ...
    def _reinit(self):
        # ensure previous run is fully gone
