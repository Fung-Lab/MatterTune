...
cpdef list grid_partition(int num_nodes,
    ...
    cdef int partitions_count = granularity * granularity * granularity
    ...
    dx = (x_max - x_min) / granularity if granularity > 0 else 1.0
    dy = (y_max - y_min) / granularity if granularity > 0 else 1.0
    dz = (z_max - z_min) / granularity if granularity > 0 else 1.0
    ...
    for i in range(num_nodes):
        ...
        partition_index = ix * (granularity * granularity) + iy * granularity + iz
