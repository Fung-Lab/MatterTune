...
OUTPUT_ROOT="${OUTPUT_ROOT:-${EXAMPLE_ROOT}/runs/02-energy-warmup}"
...
ENERGY_ONLY_EPOCHS="${ENERGY_ONLY_EPOCHS:-100}"
FORCE_RAMP_EPOCHS="${FORCE_RAMP_EPOCHS:-100}"
GRAD_PROBE_INTERVAL="${GRAD_PROBE_INTERVAL:--1}"
LOGGER="${LOGGER:-wandb}"
...
DISABLE_LR_SCHEDULER="${DISABLE_LR_SCHEDULER:-1}"
...
CMD=(
  python examples/water-energy-force-conflict/02-energy-warmup/train.py
  ...
  --energy_only_epochs "${ENERGY_ONLY_EPOCHS}"
  --force_ramp_epochs "${FORCE_RAMP_EPOCHS}"
...
echo "==================== WATER ENERGY/FORCE CONFLICT 02 ===================="
...
echo "ENERGY_ONLY_EPOCHS  = ${ENERGY_ONLY_EPOCHS}"
echo "FORCE_RAMP_EPOCHS   = ${FORCE_RAMP_EPOCHS}"
