...
from mattertune.wrappers.utils.parallel_inference import (
    ParallizedInferenceMultiWatcher,
...
def fix_bottom_layers(
    ...
    TiO_indices = [i for i, s in enumerate(symbols) if s in ("Ti", "O")]
    TiO_positions = np.array(atoms.get_positions())[TiO_indices]
    symbols = atoms.get_chemical_symbols()
    

    sorted_indices_by_z = np.argsort(TiO_positions[:, 2])
    bottom_layers_indices = sorted_indices_by_z.reshape(num_layers, -1)[0:num_bottom_layers].reshape(-1)
    bottom_layers_indices = [TiO_indices[i] for i in bottom_layers_indices]
    
    ## set bottom layers to be fixed
    fix = FixAtoms(indices=bottom_layers_indices)
...
def main(args_dict: dict):
    ...
    ## init_struct is a .xyz file with m=?_n=?_k=?_r=? in the name
    filename = args_dict["init_struct"].split("/")[-1].replace(".xyz", "")
    m, n, k, r = None, None, None, None
    ...
    assert m is not None and n is not None and k is not None and r is not None, "Invalid init_struct filename"
    print(f"m={m}, n={n}, k={k}, r={r}")
    atoms = fix_bottom_layers(
        atoms=atoms,
        num_layers=k,
        num_bottom_layers=k-m,
    )
    ...
    average_partition_time = np.mean(calc.partition_times)
    average_forward_time = np.mean(calc.forward_times)
    average_collect_time = np.mean(calc.collect_times)
    average_partition_size = np.mean(calc.partition_sizes)
    ...
    rich.print(f"Average Partition Time: {average_partition_time:.4f} seconds")
    rich.print(f"Average Forward Time: {average_forward_time:.4f} seconds")
    rich.print(f"Average Collect Time: {average_collect_time:.4f} seconds")
    rich.print(f"Average Partition Size: {average_partition_size:.4f} atoms")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt_path",
        type=str,
        default="/net/csefiles/coc-fung-cluster/lingyu/checkpoints/PtTiO-checkpoints/mattersim-1m-best-MPx1.ckpt",
    )
    ...
    parser.add_argument("--init_struct", type=str, default="./init_structs/Pt_TiO2_m=6_n=6_k=8_r=0.125.xyz")
    # parser.add_argument("--init_struct", type=str, default="./init_structs/Pt_TiO2.xyz")
