...
def main(args_dict: dict):
    def hparams():
        ...
        if args_dict["model_type"] == "mattersim-1m":
            ...
            # hparams.model.graph_convertor.threebody_cutoff = 2.0
        ...
        conservative = "mattersim" in args_dict["model_type"] or args_dict["conservative"]
        ...
        hparams.data.train.src = f"/net/csefiles/coc-fung-cluster/lingyu/datasets/SiOx-train-{args_dict['down_sample']}%.xyz"
        hparams.data.validation = MC.XYZDatasetConfig.draft()
        hparams.data.validation.src = "/net/csefiles/coc-fung-cluster/lingyu/datasets/SiOx-val.xyz"
        ...
        hparams.trainer.early_stopping = MC.EarlyStoppingConfig(
            monitor=f"val/forces_mae", patience=args_dict["early_stop_patience"], mode="min"
        )
    ...
    if is_rank_zero():
        ...
        test_files = [
            ...
            "./data/a-SiO2-chikmd.xyz",
            "./data/a-SiO2-gapmd.xyz",
            "./data/a-SiO2-acemd.xyz",
            "./data/a-SiO2-surfaces.xyz",
            "./data/a-Si.xyz",
            "./data/a-SiOx.xyz",
            "./data/a-SiO2-high-p.xyz",
        ]
        
        calc = ft_model.ase_calculator(
            device = f"cuda:{args_dict['devices'][0]}"
        )
        rich.print(f"Training on {args_dict['down_sample']}% of the dataset")
        for file in test_files:
            test_atoms_list: list[Atoms] = read(file, ":")  # type: ignore
            forces_true, stresses_true = [], []
            forces_pred, stresses_pred = [], []
            e_sqerr_sum = 0.0      # Σ (E_pred - E_true)^2
            natoms_sum  = 0        # Σ N_atoms
            pbar = tqdm(total=len(test_atoms_list), desc=f"Evaluating {file.split('/')[-1]}")
            for atoms in test_atoms_list:
                n = len(atoms)
                e_true   = atoms.info["dft_energy"]                # eV
                f_true   = atoms.arrays["dft_forces"]              # (n, 3)
                s_true   = atoms.info["dft_stress"]                # (3, 3)
                forces_true.extend(np.asarray(f_true).tolist())
                stresses_true.extend(np.asarray(s_true).tolist())
                atoms.set_calculator(calc)
                e_pred = atoms.get_potential_energy()              # eV
                f_pred = atoms.get_forces()                        # (n, 3)
                s_pred = atoms.get_stress(voigt=False)             # (3, 3)
                forces_pred.extend(np.asarray(f_pred).tolist())
                stresses_pred.extend(np.asarray(s_pred).tolist())
                e_sqerr_sum += (e_pred - e_true) ** 2
                natoms_sum  += n
                pbar.update(1)
            pbar.close()
            # Energy RMSE → meV/atom
            e_rmse = (e_sqerr_sum / natoms_sum) ** 0.5 * 1e3
            # Force RMSE → eV/Å
            f_rmse = torch.sqrt(
                torch.mean(
                    (torch.tensor(forces_pred,  dtype=torch.float64) -
                    torch.tensor(forces_true, dtype=torch.float64)) ** 2
                )
            ).item()
            s_rmse = torch.sqrt(
                torch.mean(
                    (torch.tensor(stresses_pred,  dtype=torch.float64) -
                    torch.tensor(stresses_true, dtype=torch.float64)) ** 2
                )
            ).item()
            s_rmse = s_rmse/units.GPa  # Convert stress RMSE to eV/Å³

            dataset_name = file.split("/")[-1].split(".")[0]
            rich.print(
                f"[bold cyan]Dataset:[/] {dataset_name}  "
                f"[bold]Energy RMSE:[/] {e_rmse:.2f} meV/atom  "
                f"[bold]Force RMSE:[/] {f_rmse:.4f} eV/Å  "
                f"[bold]Stress RMSE:[/] {s_rmse:.4f} GPa\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conservative", action="store_true", help="Use conservative forces for training")
    args = parser.parse_args()
    args_dict = vars(args)
    
    args_dict["down_sample"] = int(args_dict["down_sample"]*100)
