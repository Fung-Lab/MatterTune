...
usage() {
  cat <<'EOF'
Usage:
  bash examples/elec-Li-new/01-train-pair-100-uma/train.sh [train.py options]

Environment overrides:
  MODEL_TYPE=uma
  FORCE_MODE=direct|conservative
  ...
  TRAIN_FILE TEST_FILE OUTPUT_ROOT OUTPUT_DIR RESUME_CHECKPOINT
  DEVICES BATCH_SIZE NUM_WORKERS LR MAX_EPOCHS TRAIN_SPLIT
  E_LOSS_WEIGHT F_LOSS_WEIGHT LOGGER WANDB_PROJECT WANDB_NAME
EOF
}
...
case "${FORCE_MODE}" in
  direct|direct-force|direct-forces|nonconservative|non-conservative)
    FORCE_MODE="direct"
    ;;
  conservative|conservative-force|conservative-forces)
    FORCE_MODE="conservative"
    ;;
  *)
    echo "Unsupported FORCE_MODE=${FORCE_MODE}; expected direct or conservative." >&2
    exit 2
    ;;
esac
...
MODEL_LABEL="${MODEL_LABEL// /_}"
TRAIN_FILE="${TRAIN_FILE:-${TRAIN_DATA_ROOT}/Li_system_lambda_parent_del_pairs.xyz}"
TEST_FILE="${TEST_FILE:-${TEST_DATA_ROOT}/Li_system_test_with_del.xyz}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${TRAIN_DATA_ROOT}/local_runs/01-train-pair-100-uma}"
RUN_STAMP="${RUN_STAMP:-$(date +%Y%m%d-%H%M%S)}"
RUN_NAME="${RUN_NAME:-${RUN_STAMP}-${MODEL_LABEL}-${FORCE_MODE}-pair100-efonly-fw20}"
...
REFERENCE_ROOT="${REFERENCE_ROOT:-${OUTPUT_ROOT}/references}"
ENERGY_REFERENCE="${ENERGY_REFERENCE:-${REFERENCE_ROOT}/Li_system_lambda_parent_del_pairs-${MODEL_LABEL}-${FORCE_MODE}-${REFERENCE_ENERGY_SOURCE}-residual-${REFERENCE_MODEL}-alpha${RIDGE_ALPHA}.json}"
REFIT_REFERENCE="${REFIT_REFERENCE:-0}"
REFERENCE_DEVICE="${REFERENCE_DEVICE:-cuda:0}"
REFERENCE_BATCH_SIZE="${REFERENCE_BATCH_SIZE:-${BATCH_SIZE:-1}}"
...
F_LOSS_WEIGHT="${F_LOSS_WEIGHT:-5.0}"
MONITOR="${MONITOR:-val/total_loss}"
PATIENCE="${PATIENCE:-100}"
LOGGER="${LOGGER:-wandb}"
WANDB_PROJECT="${WANDB_PROJECT:-MatterTune-Electrolyte-Li-pair-100-UMA}"
...
if [[ "${REFIT_REFERENCE}" == "1" || ! -f "${ENERGY_REFERENCE}" ]]; then
  REF_CMD=(
    python examples/elec-Li-new/01-train-pair-100-uma/fit_residual_reference.py
    --xyz_path "${TRAIN_FILE}"
...
TRAIN_CMD=(
  python examples/elec-Li-new/01-train-pair-100-uma/train.py
  --model_type "${PY_MODEL_TYPE}"
  --model_name "${MODEL_NAME}"
  --task_name "${TASK_NAME}"
  --force_mode "${FORCE_MODE}"
...
echo "==================== TRAIN MLIP WITH DELETED LI (E/F ONLY) ===================="
...
echo "SUPERVISION      = energy + forces"
echo "LOSS             = MSE"
