...
EXPERIMENT_ROOT="${EXPERIMENT_ROOT:-${TRAIN_DATA_ROOT}/local_runs/01-train-pair-100-uma}"
...
DRY_RUN="${DRY_RUN:-0}"
CKPT="${CKPT:-/net/csefiles/coc-fung-cluster/lingyu/Li-electrolyte-100/local_runs/01-train-pair-100-uma/20260603-225022-uma-uma-s1.1-conservative-pair100-efonly-fw20/checkpoints/last.ckpt}"
FORCE_MODE="${FORCE_MODE:-}"
FORCE_MODE="${FORCE_MODE//_/-}"
case "${FORCE_MODE}" in
  ""|direct|conservative) ;;
  direct-force|direct-forces|nonconservative|non-conservative)
    FORCE_MODE="direct"
    ;;
  conservative-force|conservative-forces)
    ...
    ;;
  *)
    echo "FORCE_MODE must be empty, direct, or conservative; got ${FORCE_MODE}" >&2
    exit 2
    ;;
esac
RUN_DIR="${RUN_DIR:-}"
RUN_STAMP="${RUN_STAMP:-$(date +%Y%m%d-%H%M%S)}"
MLIP_LABEL="${MLIP_LABEL:-MLIP-MD pair100 UMA${FORCE_MODE:+ ${FORCE_MODE}}}"

usage() {
  cat <<'EOF'
Usage:
  bash examples/elec-Li-new/01-train-100-uma/run_Tx.sh [--tasks T0,T2,T3] [options]
...
Options:
  ...
  --checkpoint PATH     Checkpoint for T1/T2. Defaults to latest UMA pair100 best ckpt under CHECKPOINT_ROOT.
  --lambda-value VALUE  Lambda value for T2 and output file names. Default: 1.0.
  --dry-run            Print commands without executing them.

Environment overrides:
  EXPERIMENT_ROOT CHECKPOINT_ROOT RUN_ROOT FORCE_MODE CONFIG_TYPE STRUCTURE AIMD_XYZ AIMD_DAT TARGET_INDICES DEVICE
...
resolve_checkpoint() {
  ...
  if [[ -n "${FORCE_MODE}" ]]; then
    find "${CHECKPOINT_ROOT}" \
      -path "*/checkpoints/*-${FORCE_MODE}-pair100-best.ckpt" \
      -printf '%T@ %p\n' 2>/dev/null \
      | sort -nr \
      | head -n 1 \
      | cut -d' ' -f2-
...
if [[ -z "${RUN_DIR}" ]]; then
  RUN_DIR="${RUN_ROOT}/${CONFIG_TYPE}/${RUN_STAMP}-mtpair100-uma${FORCE_MODE:+-${FORCE_MODE}}-${LAMBDA_TAG}-${TOTAL_TIME_TAG}ps"
fi
...
echo "TASKS=${TASKS}"
echo "FORCE_MODE=${FORCE_MODE:-any}"
...
if task_enabled T0; then
  run_cmd python examples/elec-Li-new/01-train-100-uma/T0_split_config_summary.py \
    --out-dir "${RUN_DIR}/T0_split"
fi

if task_enabled T1; then
  run_cmd python examples/elec-Li-new/01-train-100-uma/T1_evaluate_checkpoint.py \
    --checkpoint "${CKPT}" \
    --device "${DEVICE}" \
...
if task_enabled T2; then
  ...
  run_cmd python examples/elec-Li-new/01-train-100-uma/T2_run_lambda_md.py \
    --checkpoint "${CKPT}" \
...
if task_enabled T3; then
  ...
  run_cmd python examples/elec-Li-new/01-train-100-uma/T3_plot_energy_compare.py \
...
if task_enabled T4; then
  ...
  run_cmd python examples/elec-Li-new/01-train-100-uma/T4_plot_rdf_compare.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --max-time-ps "${MAX_TIME_PS}" \
...
if task_enabled T5; then
  ...
  run_cmd python examples/elec-Li-new/01-train-100-uma/T5_plot_mol_com_rdf_compare.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --top-pdb "${TOP_PDB}" \
    --max-time-ps "${MAX_TIME_PS}" \
...
if task_enabled T6; then
  ...
  run_cmd python examples/elec-Li-new/01-train-100-uma/T6_plot_mol_com_rdf_windows.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --top-pdb "${TOP_PDB}" \
...
if task_enabled T7; then
  ...
  run_cmd python examples/elec-Li-new/01-train-100-uma/T7_plot_mol_com_rdf_window_overlay.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --top-pdb "${TOP_PDB}" \
    --total-time-ps "${T6_TOTAL_TIME_PS}" \
