...
usage() {
  cat <<'EOF'
Usage:
  bash examples/elec-Li-new/03-train-pair-100-uma/train.sh [train_delta.py options]
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${TRAIN_DATA_ROOT}/local_runs/03-train-pair-100-uma}"
RUN_STAMP="${RUN_STAMP:-$(date +%Y%m%d-%H%M%S)}"
RUN_NAME="${RUN_NAME:-${RUN_STAMP}-${MODEL_LABEL}-${FORCE_MODE}-pair100-fw20-de05}"
...
if [[ "${REFIT_REFERENCE}" == "1" || ! -f "${ENERGY_REFERENCE}" ]]; then
  REF_CMD=(
    python examples/elec-Li-new/03-train-pair-100-uma/fit_residual_reference.py
    --xyz_path "${TRAIN_FILE}"
...
TRAIN_CMD=(
  python examples/elec-Li-new/03-train-pair-100-uma/train_delta.py
  --model_type "${PY_MODEL_TYPE}"
  --model_name "${MODEL_NAME}"
  --task_name "${TASK_NAME}"
  --force_mode "${FORCE_MODE}"
