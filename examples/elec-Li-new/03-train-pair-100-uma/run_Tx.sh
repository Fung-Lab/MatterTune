...
EXPERIMENT_ROOT="${EXPERIMENT_ROOT:-${TRAIN_DATA_ROOT}/local_runs/03-train-pair-100-uma}"
...
usage() {
  cat <<'EOF'
Usage:
  bash examples/elec-Li-new/03-train-pair-100-uma/run_Tx.sh [--tasks T0,T2,T3] [options]
...
if task_enabled T0; then
  run_cmd python examples/elec-Li-new/03-train-pair-100-uma/T0_split_config_summary.py \
    --out-dir "${RUN_DIR}/T0_split"
fi

if task_enabled T1; then
  run_cmd python examples/elec-Li-new/03-train-pair-100-uma/T1_evaluate_checkpoint.py \
    --checkpoint "${CKPT}" \
    --device "${DEVICE}" \
...
if task_enabled T2; then
  ...
  run_cmd python examples/elec-Li-new/03-train-pair-100-uma/T2_run_lambda_md.py \
    --checkpoint "${CKPT}" \
...
if task_enabled T3; then
  ...
  run_cmd python examples/elec-Li-new/03-train-pair-100-uma/T3_plot_energy_compare.py \
...
if task_enabled T4; then
  ...
  run_cmd python examples/elec-Li-new/03-train-pair-100-uma/T4_plot_rdf_compare.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --max-time-ps "${MAX_TIME_PS}" \
...
if task_enabled T5; then
  ...
  run_cmd python examples/elec-Li-new/03-train-pair-100-uma/T5_plot_mol_com_rdf_compare.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --top-pdb "${TOP_PDB}" \
    --max-time-ps "${MAX_TIME_PS}" \
...
if task_enabled T6; then
  ...
  run_cmd python examples/elec-Li-new/03-train-pair-100-uma/T6_plot_mol_com_rdf_windows.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --top-pdb "${TOP_PDB}" \
...
if task_enabled T7; then
  ...
  run_cmd python examples/elec-Li-new/03-train-pair-100-uma/T7_plot_mol_com_rdf_window_overlay.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --top-pdb "${TOP_PDB}" \
    --total-time-ps "${T6_TOTAL_TIME_PS}" \
