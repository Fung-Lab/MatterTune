...
import T3_plot_mol_com_rdf_compare as t3
...
def read_frame_time_ps(comment: str, *, frame_idx: int, dt_fs: float | None, path: Path) -> float:
    time_ps = t3.parse_comment_time_ps(comment)
    ...
    with path.open("r", encoding="utf-8") as handle:
        ...
        while True:
            ...
            for atom_idx in range(natoms):
                positions[atom_idx] = t3.parse_position_line(handle.readline())

            center_positions = t3.molecule_centers(
                positions,
                molecules,
                box_length=box_length,
                reference_mode="mol_com",
            )
            neighbor_positions = t3.molecule_centers(
                positions,
                molecules,
                box_length=box_length,
            ...
            for center_name, center_ids in center_groups.items():
                ...
                for neighbor_name, neighbor_ids in neighbor_groups.items():
                    ...
                    distances = t3.minimum_image_pair_distances(
                        centers,
                        neighbors,
...
def plot_pair_windows(
    ...
    *,
    ...
    center_name, neighbor_name = key
    neighbor_label = t3.reference_mode_label(neighbor_name, str(aimd["neighbor_center"]))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--aimd-xyz", type=Path, default=t3.DEFAULT_AIMD_XYZ)
    parser.add_argument("--mlip-xyz", type=Path, required=True)
    parser.add_argument("--top-pdb", type=Path, default=t3.DEFAULT_TOP_PDB)
    ...
    parser.add_argument("--prefix", default="T4_mol_com_rdf_windows")
    ...
    try:
        args.neighbor_center = t3.normalize_reference_mode(args.neighbor_center)
...
def main() -> None:
    args = parse_args()
    center_resnames = t3.parse_csv_list(args.center_resnames)
