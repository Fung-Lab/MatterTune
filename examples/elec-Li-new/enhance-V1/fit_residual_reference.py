...
DEFAULT_DATA_ROOT = Path("/net/csefiles/coc-fung-cluster/lingyu/Li-electrolyte-V1")
DEFAULT_TRAIN_FILE = DEFAULT_DATA_ROOT / "Li_electrolyte_V1_all.xyz"
...
def create_training_energy_model(
    *,
    ...
    if model_type == "uma":
        # UMA changes the raw energy head when conservative forces use the
        # energy+force head, so the residual reference is force-mode specific.
        model_config.properties.append(
            MC.ForcesPropertyConfig(
                loss=MC.MSELossConfig(),
                loss_coefficient=1.0,
                conservative=normalize_force_mode(force_mode) == "conservative",
            )
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="UMA force-head mode to mirror while fitting training-head references.",
    )
