...
RDF_LAST_FRACTION="${RDF_LAST_FRACTION:-0.50}"
...
if [[ -z "${MLIP_EF_MODE+x}" ]]; then
  MLIP_EF_MODE="${ENERGY_MODE:-with_lj}"
fi
...
usage() {
  cat <<'EOF'
Usage:
  bash examples/elec-Li-withDel-04/run_T0_T5.sh [--tasks T0,T2,T3] [options]

Options:
  --tasks LIST          Comma-separated list: T0,T1,T2,T3,T4,T5,T6, or all.
  --run-dir PATH        Existing/new run directory. Required when running T3/T4/T5/T6 without T2 outputs in the default new directory.
  --checkpoint PATH     Checkpoint for T1/T2. Defaults to latest elec-Li-withDel-04 best ckpt.
...
resolve_checkpoint() {
  ...
  find /net/csefiles/coc-fung-cluster/lingyu/electrolyte/local_runs/elec-Li-withDel-04 \
...
if [[ -z "${RUN_DIR}" ]]; then
  RUN_DIR="${RUN_ROOT}/${CONFIG_TYPE}/${RUN_STAMP}-mt04-${LAMBDA_TAG}-${TOTAL_TIME_TAG}ps"
fi
...
if task_enabled T0; then
  run_cmd python examples/elec-Li-withDel-04/T0_split_config_summary.py \
    --out-dir "${RUN_DIR}/T0_split"
fi

if task_enabled T1; then
  run_cmd python examples/elec-Li-withDel-04/T1_evaluate_checkpoint.py \
    --checkpoint "${CKPT}" \
    --device "${DEVICE}" \
...
if task_enabled T2; then
  ...
  run_cmd python examples/elec-Li-withDel-04/T2_run_lambda_md.py \
    --checkpoint "${CKPT}" \
...
if task_enabled T3; then
  ...
  run_cmd python examples/elec-Li-withDel-04/T3_plot_energy_compare.py \
    ...
    --mlip-label "MLIP-MD 04 ${LAMBDA_TAG} first ${MAX_TIME_PS} ps" \
    --title "${LAMBDA_TAG}: AIMD vs MLIP-MD 04, matched early-time window" \
...
if task_enabled T4; then
  ...
  run_cmd python examples/elec-Li-withDel-04/T4_plot_rdf_compare.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --max-time-ps "${MAX_TIME_PS}" \
...
if task_enabled T5; then
  ...
  run_cmd python examples/elec-Li-withDel-04/T5_plot_mol_com_rdf_compare.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --top-pdb "${TOP_PDB}" \
    --max-time-ps "${MAX_TIME_PS}" \
...
if task_enabled T6; then
  ...
  run_cmd python examples/elec-Li-withDel-04/T6_plot_mol_com_rdf_windows.py \
    --mlip-xyz "${MLIP_XYZ}" \
    --aimd-xyz "${AIMD_XYZ}" \
    --top-pdb "${TOP_PDB}" \
    ...
    --aimd-label "AIMD" \
    --mlip-label "MLIP-MD 04 ${LAMBDA_TAG}" \
    --target-indices "${RDF_TARGET_INDICES}" \
    --center-resnames "${MOL_CENTER_RESNAMES}" \
    --neighbor-resnames "${MOL_NEIGHBOR_RESNAMES}" \
    --center-mode "${MOL_RDF_CENTER_MODE}" \
