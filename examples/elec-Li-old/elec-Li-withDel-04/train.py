...
def build_config(args: argparse.Namespace):
    ...
    ckpt_name = f"{args.model_name}-withDel-best"
    ...
    run_name = args.wandb_name or f"{datetime.now().strftime('%Y%m%d-%H%M%S')}-mattersim-withDel"
    config_snapshot = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wandb_project", default="MatterTune-Electrolyte-Li-withDel-04")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Reset MatterSim output head before finetuning. Default is false "
            "so the residual reference remains aligned with the pretrained model."
        ),
    )
    parser.add_argument(
    ...
    run_name = args.wandb_name or f"{datetime.now().strftime('%Y%m%d-%H%M%S')}-mattersim-withDel-04"
