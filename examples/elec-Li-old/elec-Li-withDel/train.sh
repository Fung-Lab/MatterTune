...
OUTPUT_ROOT="${OUTPUT_ROOT:-${DATA_ROOT}/local_runs/elec-Li-withDel}"
...
LOGGER="${LOGGER:-wandb}"
WANDB_PROJECT="${WANDB_PROJECT:-MatterTune-Electrolyte-Li-withDel}"
...
if [[ "${REFIT_REFERENCE}" == "1" || ! -f "${ENERGY_REFERENCE}" ]]; then
  REF_CMD=(
    python examples/elec-Li-withDel/fit_residual_reference.py
    --xyz_path "${TRAIN_FILE}"
...
TRAIN_CMD=(
  python examples/elec-Li-withDel/train.py
  --model_name "${MODEL_NAME}"
