...
DEFAULT_PAIR_FILE = (
    ...
    / "elec-Li-withDel-02"
    / "data"
    / "delta_pairs_train-parent_train.xyz"
)
...
def deleted_parent_config_frame(atoms: Atoms) -> tuple[str, int]:
    for config_key in ("delta_pair_parent_config_type", "deleted_parent_config_type"):
    ...
    for frame_key in ("delta_pair_parent_frame", "deleted_parent_frame"):
...
def load_pair_split_records(
    ...
    *,
    ...
    for pair_index in range(n_pairs):
        ...
        if int(normal.info.get("delta_pair_role", -1)) != 0:
            raise ValueError(f"Expected normal role at pair {pair_index}")
        if int(deleted.info.get("delta_pair_role", -1)) != 1:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Keep only train/val pairs whose normal parent frame is at or below this value. Set <0 to disable.",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
