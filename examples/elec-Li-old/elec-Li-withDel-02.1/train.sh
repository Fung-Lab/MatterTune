...
OUTPUT_ROOT="${OUTPUT_ROOT:-${DATA_ROOT}/local_runs/elec-Li-withDel-02.1}"
RUN_STAMP="${RUN_STAMP:-$(date +%Y%m%d-%H%M%S)}"
RUN_NAME="${RUN_NAME:-${RUN_STAMP}-mattersim-withDel-02.1-first100}"
...
LOGGER="${LOGGER:-wandb}"
WANDB_PROJECT="${WANDB_PROJECT:-MatterTune-Electrolyte-Li-withDel-02.1}"
...
if [[ "${REFIT_REFERENCE}" == "1" || ! -f "${ENERGY_REFERENCE}" ]]; then
  REF_CMD=(
    python examples/elec-Li-withDel-02.1/fit_residual_reference.py
    --xyz_path "${TRAIN_FILE}"
...
if [[ "${REBUILD_PAIR_TRAIN_FILE}" == "1" ]]; then
  PAIR_CMD=(
    python examples/elec-Li-withDel-02.1/prepare_delta_pairs.py
...
TRAIN_CMD=(
  python examples/elec-Li-withDel-02.1/train_delta.py
  --model_name "${MODEL_NAME}"
...
echo "MAX_PARENT_FRAME = ${MAX_PARENT_FRAME} (inclusive)"
