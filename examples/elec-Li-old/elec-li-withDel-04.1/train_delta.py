...
DEFAULT_PAIR_TRAIN_FILE = (
    DATA_ROOT / "local_runs" / "elec-Li-withDel-02" / "data" / "delta_pairs_train-parent_train.xyz"
)
DEFAULT_OUTPUT_ROOT = DATA_ROOT / "local_runs" / "elec-li-withDel-04.1"
DEFAULT_INIT_CHECKPOINT = (
    DATA_ROOT
    / "local_runs"
    / "elec-Li-withDel-02"
    / "20260512-parentSplit-02"
    / "checkpoints"
...
class DeltaPairDataModule(LightningDataModule):
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        if self.max_parent_frame is not None:
            ...
            for index in range(0, len(atoms_list), 2):
                parent_frame = int(atoms_list[index].info["frame"])
                if parent_frame < self.max_parent_frame:
    ...
    def _mapped_dataset(self) -> MapDatasetWrapper[Atoms, Any]:
        ...
        def map_fn(atoms: Atoms):
            ...
            delta_pair_id = torch.tensor([int(atoms.info["delta_pair_id"])], dtype=torch.long)
            delta_pair_role = torch.tensor([int(atoms.info["delta_pair_role"])], dtype=torch.long)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wandb_project", default="MatterTune-Electrolyte-Li-withDel-04.1")
    ...
    run_name = args.wandb_name or (
        ...
        f"{model_label(args.model_type, args.model_name)}-withDel-04.1-first100-fw20-de05"
    )
