...
def build_config(args: argparse.Namespace):
    ...
    ckpt_name = f"{model_label(args.model_type, args.model_name)}-withDel-best"
    ...
    run_name = args.wandb_name or (
        ...
        f"{model_label(args.model_type, args.model_name)}-withDel"
    )
    config_snapshot = {
...
def parse_args() -> argparse.Namespace:
    ...
    run_name = args.wandb_name or (
        f"{datetime.now().strftime('%Y%m%d-%H%M%S')}-"
        f"{model_label(args.model_type, args.model_name)}-withDel-04.1"
    )
